    ...
    */
    ...
    visualize() {
        ...
        this._svg = d3.select(container)
            .select("svg")
            .append("g")
            .attr("transform", margin.top === 130 * this.scaleratio ? "translate(" + (Wscaleratio*640-this.width())/2 + "," + (Hscaleratio*640-this.height())/2 + ")" : "translate(" + (Wscaleratio*640-this.width())/2  + "," + (Hscaleratio*640-this.height())/2 + ")");
        ...
        if (background.Background_Image) {
            ...
            defs.append("svg:pattern")
                .attr("class", "svg-backgroundimage")
                .attr("width", 1)
                .attr("height", 1)
        ...
        if (this.style()['background-color']) {
            ...
            let defs = d3.select(container).select("svg").append('svg:defs');
            defs.append("svg:pattern")
                .attr("class", "chart-backgroundimage")
                .attr("width", 1)
                .attr("height", 1)
                .attr("patternUnits", "objectBoundingBox")
                .append("svg:image")
        ...
        if (this.units.length<20){
            this.radiusMultiplier = 3
        }
        else if (this.units.length<40){
            this.radiusMultiplier = 2.5
        }
        else if (this.units.length<60){
    ...
    */
    ...
    initvis() {
        ...
        content.append("g")
            ...
            //         console.log('111111')
            //         return 6;
            //     } else if (unitsLength < 40) {
            //         console.log('222222')
            //         return 4;
            //     } else if (unitsLength < 60) {
            //         console.log('33333')
            //         return 2;
            //     } else {
            //         // 如果单位数量超过60，可以添加额外的条件或使用默认值
            //         return 2;
            //     }
            // })
