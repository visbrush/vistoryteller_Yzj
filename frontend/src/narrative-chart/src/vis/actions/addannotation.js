...
import {Arrow, Circle, Contour, Desaturate, Distribution, Fade, Fill, Glow, Hide, Label, Reference, Regression, Show, Symbol, Texture, Tooltip, Seasonality, Partialrect} from './annotations'
    ...
    _type2annotator(type) {
        switch (type) {
            ...
            case AnnotationType.TOOLTIP:
                return new Tooltip()
            case AnnotationType.SEASONALITY:
                return new Seasonality()
            case AnnotationType.PARTIALRECT:
                return new Partialrect()
            default:
                return new Fill();
        }
    }
