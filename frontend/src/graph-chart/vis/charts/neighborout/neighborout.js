...
class Neighborout extends Chart{
    constructor() {
        super();
        this._x = "";
        this._y = "";
    ...
    display(){
        ...
        const focus = this._focus[0].node
        ...
        const NT = calNeighborSource(measurefield,links,nodes)
        NT.filter((obj) => { 
            console.log(highlightStrokeColor(obj[0].group),highlightColor(obj[0].group))
...
//neighborout
const calNeighborSource = (focus,links,nodes) =>{
    let neighborSource = [];
    for (var i=0; i < links.length; i++ ){
        //console.log(links[i].target.id,focus)
        if (links[i].source.id === focus){
            var node = nodes.filter((obj)=>{
                return obj.id == links[i].target.id
            })
