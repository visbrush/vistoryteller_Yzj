...
class AccessibilityIn extends Chart {
    constructor() {
        super();
        this._x = "";
        this._y = "";
    ...
    display() {
        ...
        const reachableFromNodes = findReachableFromNodes(measurefield, links, nodes)
        console.log(reachableFromNodes, "reachableFromNodes")

        reachableFromNodes.filter((obj) => {
        ...
        // 遍历连接来绘制箭头
        const reachableFromNodeIds = reachableFromNodes.map(node => node.id);
        link.each(function (d) {
            const sourceId = d.source.id;
            const targetId = d.target.id;

            // 如果源节点和目标节点都在 reachableNodeIds 中，表示这条连接在可达路径上
            if (reachableFromNodeIds.includes(sourceId) && reachableFromNodeIds.includes(targetId)) {
                // 高亮颜色
                d3.select(this)
...
export default AccessibilityIn;